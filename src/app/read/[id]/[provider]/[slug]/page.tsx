...
async function Page({ params }: Props) {
  ...
  let List: any = [];
  let index = 0;
  while (info.data.chapters.data[index] !== undefined) {
    let index0 = 0;
    if (info.data.chapters.data[index].providerId === params.provider) {
      while (info.data.chapters.data[index].chapters[index0] !== undefined) {
        List.push(info.data.chapters.data[index].chapters[index0]);
        index0++;
      }
    }
    index++;
